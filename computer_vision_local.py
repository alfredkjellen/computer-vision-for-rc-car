...
class CarDetector:
    def __init__(self, video_file=None, flipped=True, horizontal_squares=4, vertical_squares=6, competitor_name='Player', start_angle=90):
            ...
            while True:                    
                ...
                if self.pressed('\r'):
                    if len(points) == 2:
                        ...
                        finish_line = finish_line[np.argsort(finish_line[:, 0])]
    ...
    def color_detection(self, frame, history):
        ...
        cv2.putText(self.debug_frame,f"({center[0]:.0f}, {center[1]:.0f}) {angle:.0f} deg",(center_int[0] + 25, center_int[1] + 25),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255, 0, 0),1,cv2.LINE_AA)
    ...
    def run(self):
        ...
        self.start_detection(self.competitor_name)
        self.cap.release()
        cv2.destroyAllWindows()
    ...
    def create_video_file(self, competitor):
        ...
        if not os.path.exists(competitor):
            os.makedirs(competitor)
        
        i = len(os.listdir(competitor)) + 1
        out_put_file = f'{competitor}/{competitor}_video{i}.mp4'
    ...
    def start_detection(self, competitor):            
        frame_count = 0
        history = History()
        ...
        while True:
            ...
            self.started = self.position is not None and self.angle is not None

            if not self.finished:
                has_crossed_finish_line = self.crossed_finish_line(history)
                if has_crossed_finish_line:
                    print("crossed finish line at", elapsed_time)
            ...
            if self.finished:
                if finished_at - time.time() > 1:
                    break
            
            self.debug_detection(frame_count, elapsed_time)
    ...
    def debug_detection(self, frame_count, time_):
        if self.position is not None and self.angle is not None:
            ...
            for destination in distance_points:
                ...
                cv2.line(self.debug_frame, (x, y), destination, (255, 0, 0), thickness=1)
        ...
        cv2.line(self.debug_frame, self.finish_line[0], self.finish_line[1], (0, 0, 255), 2)
        ...
        if self.finished:
            ...
            self.put_text(self.debug_frame, f"time: {time_:.2f} s", 5)
    ...
    def crossed_finish_line(self, history):
        position = history.get_latest_position()
        if position is None:
            return False
...
if __name__ == "__main__":
    ...
    #detector = CarDetector("forward.avi", flipped=False)
